...
-- Using proper joins to populate ticker_symbol from watchlist_lookup
INSERT INTO watchlist_companies (
    item_name, market, title, summary, link, published_at, impact_score, source_system, ticker_symbol
) VALUES
(
    'Apple',
    'US',
    ...
    8.5,
    'Bloomberg',
    (SELECT ticker_symbol FROM watchlist_lookup WHERE item_name = 'Apple' AND item_type = 'companies' AND market = 'US')
),
(
    'Apple',
    'US',
...
(
    ...
    9.2,
    'Reuters',
    (SELECT ticker_symbol FROM watchlist_lookup WHERE item_name = 'Tesla' AND item_type = 'companies' AND market = 'US')
),
(
    'Amazon',
    'US',
    ...
    8.9,
    'Reuters',
    (SELECT ticker_symbol FROM watchlist_lookup WHERE item_name = 'Amazon' AND item_type = 'companies' AND market = 'US')
),
(
    'Google',
    'US',
    ...
    9.5,
    'The Verge',
    (SELECT ticker_symbol FROM watchlist_lookup WHERE item_name = 'Google' AND item_type = 'companies' AND market = 'US')
);

-- Update item_id foreign keys after insertion
UPDATE watchlist_companies SET item_id = (SELECT id FROM watchlist_lookup WHERE item_name = watchlist_companies.item_name AND item_type = 'companies' AND market = watchlist_companies.market) WHERE item_id IS NULL;
